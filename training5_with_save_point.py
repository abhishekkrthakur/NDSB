...
class SavePoint(object):
    def __init__(self, interval=10, after=20):
    ...
    def __call__(self, nn, train_history):
        ...
        if current_epoch % self.interval == 0 and current_epoch > self.after:
            filename = DATABASE_FOLDER + 'sp_{}_'.format(current_epoch) + NETWORK
...
def randomize_image(img):
    ...
    tf = transform.SimilarityTransform(scale=random() * 0.2 + 0.9,
                                       translation=(int(random() * 6), int(random() * 6)))
...
net2 = NeuralNet(
    ...
    on_epoch_finished=[
        ...
        SavePoint(interval=10, after=20)
        ],
    max_epochs=500,
    verbose=2,
