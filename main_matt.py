...
def load_test_data():
    for f in glob.glob('data/test/*.jpg'):
        img = imread(f)
...
def predict():
    ...
    for i in range(0, testdata.shape[0], 4075):
    	preds = model.predict_proba(testdata[i:i+4075,:].reshape(-1, 1, IMG_SIZE, IMG_SIZE))
	...
	for i in range(45, 360, 45):
	    ...
	    for idx in range(testdata.shape[0]):
	        temp = rotate(testdata[i,:].reshape(IMG_SIZE, IMG_SIZE), 15)
	        testdata[i,:] = temp.reshape(-1)
...
idx = test_ids()
preds = np.column_stack(([str(i) + '.jpg' for i in idx], y_test))
print "creating submission file"
preds = pd.DataFrame(preds, columns=sample.columns)
preds.to_csv('submission_matt.csv', index = False)
